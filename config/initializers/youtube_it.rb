  ...
  module Parser
    ...
    protected
      #def parse_entry(entry)
        #video_id = entry.attribute("id").text
        #published_at = entry.attribute("published") ? Time.parse(entry.attribute("published").text) : nil
        #uploaded_at = entry.xpath("media:group/yt:uploaded") ? Time.parse(entry.xpath("media:group/yt:uploaded").text) : nil
        #updated_at = entry.attribute("updated") ? Time.parse(entry.attribute("updated").text) : nil
        #recorded_at = entry.xpath("yt:recorded") ? Time.parse(entry.xpath("yt:recorded").text) : nil
        ...
        #title = entry.attribute("title").text
        #html_content = nil #entry.attribute("content") ? entry.attribute("content").text : nil

        ## parse the author
        #author_element = entry.attribute("author")
        #author = nil
        #if author_element
          #author = YouTubeIt::Model::Author.new(
                     #:name => author_element.attribute("name").text,
                     #:uri => author_element.attribute("uri").text)
        #end
        #media_group = entry.xpath('media:group')

        #ytid = nil
        #unless media_group.xpath("yt:videoid").nil?
          #ytid = media_group.xpath("yt:videoid").text
        #end

        ## if content is not available on certain region, there is no media:description, media:player or yt:duration
        #description = ""
        #unless media_group.xpath("media:description").nil?
          #description = media_group.xpath("media:description").text
        #end

        ## if content is not available on certain region, there is no media:description, media:player or yt:duration
        #duration = 0
        #unless media_group.xpath("yt:duration").nil?
          #duration = media_group.xpath("yt:duration")["seconds"].to_i
        #end

        ## if content is not available on certain region, there is no media:description, media:player or yt:duration
        #player_url = ""
        #unless media_group.xpath("media:player").nil?
          #player_url = media_group.xpath("media:player")["url"]
        #end

        #unless media_group.xpath("yt:aspectRatio").nil?
          #widescreen = media_group.xpath("yt:aspectRatio").text == 'widescreen' ? true : false
        #end
