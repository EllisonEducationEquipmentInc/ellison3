...
class Product
  ...
  index [[:item_num, Mongo::ASCENDING], [:name, Mongo::ASCENDING], [:short_desc, Mongo::ASCENDING], [:systems_enabled, Mongo::ASCENDING], [:updated_at, Mongo::ASCENDING], [:active, Mongo::ASCENDING], [:deleted_at, Mongo::ASCENDING]]
  ...
  def four_related_criteria
    @four_related_criteria ||= self.class.related_to(self, self.outlet).where(:item_group => self.item_group, :tag_ids.all => [related_tag.id] + self.product_lines.map(&:id)) rescue []
  end
  ...
  def reindex?
    @marked_for_auto_indexing = self.changed? && self.changed.any? {|e| (["systems_enabled", "active", "outlet", "outlet_since", "life_cycle", "item_num", "name", "item_group", "keywords", "price_szus_usd"] + ELLISON_SYSTEMS.map {|s| ["orderable_#{s}"]}.flatten + LOCALES_2_CURRENCIES.values.map {|c| ["msrp_#{c}", "wholesale_price_#{c}"]}.flatten).include?(e)} || campaigns.any? {|c| c.reindex?} || stock_status_changed? || new_record?
