	...
	private 
		...
		def add_2_cart(product, qty = 1)
			if item = get_cart.cart_items.find_item(product.item_num)
				...
				get_cart.cart_items << CartItem.new(:name => product.name, :item_num => product.item_num, :sale_price => product.sale_price, :msrp => product.msrp_or_wholesale_price, :price => product.price, 
				  :quantity => qty, :currency => current_currency, :small_image => product.small_image, :added_at => Time.now, :product => product, :weight => product.virtual_weight, :retailer_price => product.retailer_price,
      ...
      @quote = cart_to_order_or_quote(:quote, options)
		...
		def order_to_cart(order)
		  ...
		  order.order_items.each do |item|
		    cart_item = CartItem.new(:price => item.sale_price, :small_image => item.product.small_image, :added_at => Time.now, :pre_order => item.product.pre_order?, 
		      :out_of_stock => item.product.out_of_stock?, :weight => item.product.virtual_weight, :msrp => item.product.msrp_or_wholesale_price, :minimum_quantity => item.product.minimum_quantity, 
		...
		def payment_can_be_run?
		  !(backorder_allowed? && get_cart.out_of_stock?) && (is_sizzix? || (is_er? && @order && @order.address && @order.address.us?)) && !@order.payment.try(:purchase_order)
		end
		...
		def process_order(order)
  		...
  		if admin_signed_in?
  		  order.customer_rep = current_admin.email
