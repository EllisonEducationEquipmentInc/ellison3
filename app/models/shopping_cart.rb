module ShoppingCart
  ...
  module InstanceMethods
    ...
    def process_order(order)
      ...
      order.save!
      if order.is_a?(Order) && (order.payment || order.gift_card)
    ...
    def calculate_tax(address, options={})
      ...
      total_tax = if !get_cart.gift_card? && is_us? && (calculate_tax?(address.state) || (address.country == 'Canada' && is_ee?))
    ...
    def fedex_rate(address, options={})
      Rails.logger.info "Getting Fedex rate for #{address.inspect} weight: #{options[:weight]}"
    ...
    def refund_cc_transaction(payment, options = {})
      get_gateway
      options.merge!(:order_id => "REFUND#{payment._parent.id}", :description => "REFUND#{payment._parent.id}", :currency => payment._parent.locale.to_s == 'en-UK' ? "GBP" : "EUR") if is_uk?
      options[:order_id] ||= payment._parent.id
      timeout(30) do
