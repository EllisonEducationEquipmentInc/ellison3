class Admin::QuotesController < ApplicationController
  ...
  def active_quotes_report
    ...
    unless File.exists? "/data/shared/report_files/#{filename}"
      csv_string = CSV.generate do |csv|
        csv << ["quote_number", "quote name", "item_num", "name","release_dates", "quoted_price", "sales_price", "quantity", "campaign_name",  "created_at", "expires_at", "customer_rep", "company", "name", "email", "erp", "subtotal", "shipping_amount", "handling_amount", "sales_tax", "total_amount", "total_discount", "coupon_code"]
        Quote.send(current_system).active.each do |quote|
          quote.order_items.each do |item|
            csv << [item.quote.quote_number, item.quote.name, item.item_num, item.name, item.product.tags.release_dates.map(&:name) * ', ', item.quoted_price, item.sale_price, item.quantity, item.campaign_name,  item.quote.created_at, item.quote.expires_at, item.quote.customer_rep, item.quote.user.company, item.quote.user.name, item.quote.user.email, item.quote.user.erp, item.quote.subtotal_amount, item.quote.shipping_amount, item.quote.handling_amount, item.quote.tax_amount, item.quote.total_amount, item.quote.total_discount, item.quote.coupon_code]
          end
        end
      end
      File.open("/data/shared/report_files/#{filename}", "w") {|file| file.write(csv_string)}
