class Admin::ReportsController < ApplicationController
  ...
  before_filter :admin_write_permissions!, :only => [:new, :create, :edit, :update, :destroy]
    ...
    @tags = Rails.cache.fetch("all_report_tags_#{current_system}", :expires_in => 1.hour.since) {Tag.send(current_system).where(:tag_type.in => ["designer", "artist", "special", "exclusive", "theme", "product_line"]).order_by([:tag_type, :name]).only(:tag_type).group.each_with_object({}) {|e, h| h[e["tag_type"]] = e["group"].sort {|x,y| x.name <=> y.name }.map {|i| [i.name, i.id]}}}
	end
