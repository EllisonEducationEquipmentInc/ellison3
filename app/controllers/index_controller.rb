...
class IndexController < ApplicationController
  ...
  def product
    ...
    if request.xhr?
      ...
      fresh_when(:etag => [Time.now.utc.strftime("%m%d%Y%H"), current_locale, current_system, @product,  @product.price, current_user, request.xhr?, admin_signed_in?], :last_modified => @product.updated_at.utc)
  ...
  def tag_group
    ...
    @title = "#{@klass.try(:public_name)} - #{params[:id].try(:humanize)}"
  ...
  def search
    ...
    if params[:q].present? && params[:q] =~ /^(a|A)*[0-9]{3,6}($|-[a-zA-Z0-9\.]{1,10}$)/
      @product = Product.listable.where(:item_num => params[:q].upcase).first
...
private
  ...
  def get_search
    ...
    @sort_options = idea? ? [["Relevance", nil], ["New #{Idea.public_name.pluralize}", "start_date_#{current_system}:desc"], ["#{Idea.public_name} Name [A-Z]", "sort_name:asc"], ["#{Idea.public_name} Name [Z-A]", "sort_name:desc"]] :
                      [["Relevance", nil], ["Newest", outlet? ? "outlet_since:desc" : "start_date_#{current_system}:desc"], ["Best Sellers", "quantity_sold:desc"], ["Lowest Price", "price_#{current_system}_#{current_currency}:asc"], ["Highest Price", "price_#{current_system}_#{current_currency}:desc"], ["Product Name [A-Z]", "sort_name:asc"], ["Product Name [Z-A]", "sort_name:desc"]]
