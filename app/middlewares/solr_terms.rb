...
class SolrTerms
  ...
  def call(env)
    ...
    if env["PATH_INFO"] =~ /^\/solr_terms\/(.+)$/
      ...
      term = $1
      solr_response = Net::HTTP.get(URI.parse("#{solr_path}/select?fq=type%3AProduct&q=#{term.downcase.strip.gsub(/\+$/, '')}*&fl=id+item_num_ss+stored_name_ss+msrp_usd_fs+msrp_gbp_fs+msrp_eur_fs&qf=item_num_ss+stored_name_ss&start=0&rows=20&wt=ruby&omitHeader=true&json.nl=arrarr"))
      ...
      term = $1
      solr_response = Net::HTTP.get(URI.parse("#{solr_path}/select?fq=type%3AIdea&q=#{term.downcase.strip.gsub(/\+$/, '')}*&fl=id+idea_num_ss+stored_name_ss&qf=idea_num_ss+stored_name_ss&start=0&rows=20&wt=ruby&omitHeader=true&json.nl=arrarr"))
      hash = eval(solr_response)
      ...
      term = $1
      solr_response = Net::HTTP.get(URI.parse("#{solr_path}/select?fq=type%3ATag&q=#{term.downcase.strip.gsub(/\+$/, '')}*&fl=id+stored_name_ss+tag_type_ss+systems_enabled_sms&qf=stored_name_ss&start=0&rows=20&wt=ruby&omitHeader=true&json.nl=arrarr"))
      hash = eval(solr_response)
