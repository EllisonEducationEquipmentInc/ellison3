...
class SolrTerms
  ...
  def call(env)
    ...
    if env["PATH_INFO"] =~ /^\/solr_terms\/(.+)$/
      ...
      term = $1
      solr_response = Net::HTTP.get(URI.parse("#{solr_path}/select?fq=type%3AIdea&fq=active_b%3Atrue&q=#{term.downcase.strip.gsub(/\+$/, '')}*&fl=id+idea_num_ss+stored_name_ss&qf=idea_num_ss+stored_name_ss&start=0&rows=10&wt=ruby&omitHeader=true&json.nl=arrarr"))
      hash = eval(solr_response)
      ...
      term = $1
      solr_response = Net::HTTP.get(URI.parse("#{solr_path}/select?fq=type%3ATag&fq=active_b%3Atrue&q=#{term.downcase.strip.gsub(/\+$/, '')}*&fl=id+stored_name_ss+tag_type_ss&qf=stored_name_ss&start=0&rows=10&wt=ruby&omitHeader=true&json.nl=arrarr"))
      hash = eval(solr_response)
