...
class SolrTerms
  ...
  def call(env)
    ...
    if env["PATH_INFO"] =~ /^\/solr_terms\/(.+)$/
      ...
      term = $1
      solr_response = Net::HTTP.get(URI.parse("#{solr_path}/select?fq=type%3AProduct&fq=active_b%3Atrue&q=#{term}*&fl=id+item_num_ss+stored_name_ss+msrp_usd_f&qf=item_num_ss+stored_name_ss&start=0&rows=10&wt=ruby&omitHeader=true&json.nl=arrarr"))
